...
from .enclosure.enclosure_class import Enclosure
...
class EnclosureService(CRUDService):
    ...
    def query(self, filters, options):
        ...
        if product in ['TRUENAS-M50', 'TRUENAS-M60']:
            enclosures.extend(self.middleware.call_sync('enclosure.mseries_plx_enclosures', prod))
        elif producut == 'TRUENAS-R50':
            enclosures.extend(self.middleware.call_sync('enclosure.r50_nvme_enclosures', prod))
    ...
    def __get_enclosures(self, product):
        ...
        for enc in self.middleware.call_sync('enclosure.get_ses_enclosures'):
            if enc['name'] in blacklist:
                continue
            else:
                result.append(Enclosure(enc, product))
